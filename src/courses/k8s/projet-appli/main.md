...
3. Installer un `IngressController`, par exemple `traefik` via _Helm_.
...
4. Exposer votre application à l'extérieur via un ingress (par exemple `traefik`) ou un `LoadBalancer` dédié à l'application.
5. Réfléchir à la _Scalabilité_ et la **tolérance aux pannes** :
