...
- Ajoutez le droit d'exécution pour l'utilisateur courant, retirez le droit de modification pour le groupe et retirez le droit de lecture pour les autres.
...
- Ajoutez le répertoire `mesBinaires` à la variable `$PATH`.
- Exécutez votre script en exécutant directement votre commande : `$ mesProcessus`.

## Enchaînement de commandes

- En utilisant la commande `sort`, exécutez une suite de commandes permettant de lister les utilisateurs du fichier `/etc/passwd` par ordre alphabétique.
...
- Testez le résultat de la commande suivante : `sudo echo "salut" | id`. Que remarque-t-on ? Pourquoi ?
