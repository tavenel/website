...
1. Créer un nouveau dépôt git (pour pourra utiliser l'hébergement Github® ou Gitlab® pour le partage).
...
L'un des principaux intérêts d'un serveur de code hébergé comme _GitHub_ ou _Gitlab_ est la possibilité d'interagir avec les commit _Git_ de manière assez poussée.
...
Reprendre le premier exemple en utilisant _GitHub_ ou un autre service Cloud d'hébergement de code source (_Gitlab_, …). Au moment de fusionner la branche de fonctionnalités dans le tronc commun, créer une `pull-request` préparant cette opération de fusion. Le 2e développeur aura alors la responsabilité de relire le code modifié et de proposer des changements. Intégrer ensuite la branche de fonctionnalités en acceptant la pull-request.

:::link
Pour plus d'information sur la revue de code, voir la documentation de _GitHub_ : <https://github.com/features/code-review> et celle de _Gitlab_ : <https://docs.gitlab.com/development/code_review/>.
:::

_Les outils d'hébergement de dépôts Git comme GitHub ou Gitlab permettent également de lier des pull-request à des "issues", ce qui permet d'identifier des commit, branches ou pull-request à un bug._
...
1. Créer un nouveau projet sur _Github_ ou _Gitlab_ et le partager avec les deux développeurs.
...
echo '## On effectue une "Merge-Request" sur GitHub ou Gitlab de fix-typo vers v1 et on la valide.'
