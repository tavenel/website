---
title: Utiliser un dépôt distant - GitLab
date: 2025 / 2026
tags:
- git
- gitlab
...
Pour ce TP, nous allons utiliser la plateforme `GitLab` pour y héberger notre dépôt `git`.
...
Suivre la page de tutoriel à [cette adresse](https://docs.gitlab.com/ee/gitlab-basics/create-project.html) pour créer un compte GitLab et un premier dépôt distant si ce n'est déjà fait.
...
```sh
git clone https://gitlab.com/mon_utilisateur/mon_depot_distant.git
```
...
1. En utilisant l'interface graphique de `GitLab`, réaliser un changement dans le fichier `index.html`.
...
Vérifier sur l'interface de `GitLab` que le résultat du merge a bien été envoyé à distance.
...
1. Configurer le dépôt `GitLab` d'un autre apprenant pour pouvoir y effectuer des opérations.
2. Cloner ce dépôt.
